...
- 下一版 release candidate：`1.2.1`
- Build：`3`
- 已發佈版本：`1.2.0`／Build `3`
- Team：`9CFPAUL5N5`
- 目前已發佈 App 價格：付費下載；修復版與第一個 non-consumable IAP 核准前不得改為免費
- 修復版核准後的目標 App 價格：免費下載（free app price）
...
- `NOT CONFIGURED EXTERNALLY`：本文件只準備 submission checklist；尚未建立 App Store Connect IAP、免費 App 價格、IAP 價格排程、促銷代碼或修復版送審資料。
...
- `1.2.0 HISTORICAL — VERIFIED LOCALLY`：完整 Swift 測試 853 項、靜態 release audit、iOS／macOS／Watch／Share clean builds 與 Share／Watch 嵌入稽核均通過；詳細證據見 `Verification/PatternLibraryVerification.md`，不得當作 1.2.1／Build 3 證據。
- `1.2.0 HISTORICAL — VERIFIED ON DEVICE`：簽章版本已在 iPhone 17 Pro Max 完成 Files PDF Share Sheet 顯示、匯入與重複匯入驗收；詳細證據見 `Verification/ShareExtensionActivationVerification.md`。
- `MANUAL ACCEPTANCE INCOMPLETE`：iPhone 其餘織圖庫操作、iPad 直橫向 PDF 閱讀與標註、macOS、備份還原及 VoiceOver 的 1.2 完整人工矩陣仍待執行。
- `RELEASED`：iOS／iPadOS／Apple Watch 1.2.0／Build 3 已發佈；這是修復版之前的已上架版本。
- `LATEST RECORDED REVIEW STATE`：macOS 1.2.0／Build 3 仍在等待審查。
- `PENDING REPAIR`：1.2.1／Build 3 尚未 archive、Validate、上傳或送審；完成本機 release verification 不代表已取得送審授權。
...
- `IN PROGRESS`：目前正從全部 175 個 storefronts 移除公開供應；這是暫停有問題版本供應，不是改價或修復版送審。
- `SAFETY GATE`：修復版 binary 與第一個 non-consumable IAP 均獲 Apple 核准前，不得把 App 改為免費或恢復任何 storefront 的公開供應。
- `NOT AUTHORIZED`：除上述移除公開供應外，其餘外部寫入尚未執行；不得將舊版 App 售價排程視為新 IAP 的價格設定。

## Builds／建置版本

- `CANDIDATE`：iOS／iPadOS／watchOS、Share Extension 與 macOS 的設定版本均為 `1.2.1`、Build `3`。
- `PENDING VERIFICATION`：1.2.1／Build 3 的 entitlement 整合後聚焦測試與平台 build 證據必須重新產生；不得沿用 1.2.0／Build 3 或 1.2.1／Build 2 的結果。
- `NOT UPLOADED`：1.2.1／Build 3 尚未上傳 App Store Connect。
- `RELEASED`：iOS／iPadOS／Apple Watch 1.2.0／Build 3 已發佈；macOS 1.2.0／Build 3 的最新紀錄仍為等待審查。
...
- `NOT SAVED EXTERNALLY`：1.2.1 雙語 metadata 尚未寫入 App Store Connect。
- `1.2.0 HISTORICAL — SAVED`：繁體中文與英文（美國）版本 metadata 與「此版本的新增功能」已儲存，英文商店名稱為 `KnitNote: Knitting Companion`；這不代表 1.2.1 新文案已上傳。
...
- `RELEASED`：iOS／iPadOS／Apple Watch 1.2.0／Build 3 已發佈。
- `LATEST RECORDED REVIEW STATE`：macOS 1.2.0／Build 3 已於 2026-07-28 12:09（Asia/Taipei）提交，提交 ID 為 `1d7d71a6-de1e-4b2a-9cf9-61b5c24f2865`，目前仍記錄為等待審查。
- `PENDING REPAIR`：1.2.1／Build 3 尚未送審或發佈。
- `AVAILABILITY REMOVAL IN PROGRESS`：目前正從全部 175 個 storefronts 移除公開供應；修復版與 IAP 核准前不得恢復。
...
- `AUTHORIZED HISTORICALLY FOR 1.2.0`：使用者於 2026-07-28 明確要求推送並更新上架版本；1.2.0／Build 3 已依此授權完成上傳、App Store Connect 編輯與送審。
- `NOT AUTHORIZED FOR 1.2.1`：目前工作只涵蓋 1.2.1／Build 3 release candidate 的本機整合與驗證；任何 archive 上傳、App Store Connect metadata／價格／IAP 編輯、送審或發佈仍需使用者另行明確授權。
- `AUTHORIZED AVAILABILITY HOLD`：目前授權範圍只包含從全部 175 個 storefronts 移除公開供應；不包含改為免費或恢復公開供應。
