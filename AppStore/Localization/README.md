...
`KnittingTerminology.csv` is the human-reviewed terminology source of truth for KnitNote translators and String Catalog reviews. Each row expresses one semantic meaning across English, Traditional Chinese, Simplified Chinese, German, French, Japanese, Norwegian Bokmål, Swedish, Finnish, Danish, Korean, modern Greek, and Dutch. Native-speaker release sign-off is still required; the table and automated checks do not replace it.
...
For Dutch, use `project`, `patroon`, `garen`, `toerenteller`, `toer`, `steek`, `breinaald`, and `haaknaald` as the core product terms. The CSV records reviewed plural forms or stems where Dutch inflection requires them; shipping review must keep those families consistent across main-app, Watch, Share Extension, and permission copy.
