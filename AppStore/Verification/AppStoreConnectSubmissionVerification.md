# App Store Connect 送審與上架驗證

驗證日期：2026-07-24（Asia/Taipei）

App：KnitNote  
英文商店名稱：`KnitNote: Knitting Companion`
Apple ID：`6793023054`  
版本：`1.0.0`  
Build：`2`

## 已在 App Store Connect 保存

- iOS／iPadOS／Apple Watch Build 2 已選入 iOS 1.0。
- macOS Build 2 已選入 macOS 1.0。
- iOS／macOS 繁體中文與英文（美國）行銷宣傳文字、描述、關鍵字、支援 URL、行銷 URL、版本與版權。
- 繁中與英文各有 iPhone 5 張、iPad 4 張、Apple Watch 2 張與 Mac 3 張截圖，順序與核准清單一致。
- iOS／macOS 均設定為審核通過後手動發佈。
- iOS／macOS 均設定 App 不需登入，並保存以下審核聯絡資料：
  - 名字：`Chung Lung`
  - 姓氏：`Chen`
  - 電話：`+886919185007`
  - 電子郵件：`lzz.1999@icloud.com`
- 審核備註包含六組計數器、織圖 PDF／圖片匯入、高亮、手寫、頁面筆記、每排筆記、編織日記、毛線庫、計算工具、備份還原與 Apple Watch 同步測試步驟。
- iOS／Watch 與 macOS Build 2 的出口法規答案均為未使用需申報的加密演算法。
- 全球年齡分級為 4+。
- 主要類別為「生活風格」，次要類別為「工具程式」。
- 內容版權已保存為：「是，此 App 具有第三方內容的必要權利。」
- App 隱私權問卷已發佈為「不收集資料」。
- 繁體中文與英文（美國）隱私權政策 URL 均已保存：
  - `https://phillon77.github.io/KnitNote/privacy.html`

## 英文名稱處理

原英文名稱 `KnitNote` 已被其他 App 使用。使用者於 2026-07-24 核准替代名稱：

`KnitNote: Knitting Companion`

移除無效的英文在地化後重新建立，App Store Connect 已成功保存新名稱與副標題，沒有名稱衝突或頁面錯誤。

## 送審結果

- iOS 1.0／Build 2 已於 2026-07-24 上午 7:07（Asia/Taipei）提交。
- App Store Connect 提交識別碼：`152dba51-c4e1-4de2-8652-fa988558bf1b`。
- iOS 提交頁曾顯示「已提交 1 個項目」，列表狀態為「等待審查」。
- Apple 公開查詢資料已確認 iOS 1.0 於 2026-07-24 18:24:59（Asia/Taipei）上架：
  - `https://apps.apple.com/tw/app/knitnote/id6793023054`
- macOS 1.0／Build 2 已於 2026-07-24 21:44（Asia/Taipei）正式提交。
- 重新載入版本頁後，App Store Connect 顯示「正在等待審查」。

## 價格排程

- 首發價格：美國 US$2.99、台灣 NT$90。
- 首發優惠第 30 天：2026-08-22。
- App Store Connect 已確認並保存全球價格調整：
  - 生效日：2026-08-23
  - 美國：US$2.99 → US$4.99
  - 台灣：NT$90 → NT$150
  - 範圍：175 個國家或地區
  - 結束日期：無

## macOS 狀態

- macOS 1.0／Build 2 的繁中與英文 metadata、截圖、審核資訊與手動發佈設定均已完成。
- 送審前已重新確認不需登入，並恢復及儲存審核聯絡資料與 macOS 專用測試步驟。
- macOS 1.0 已加入提交草稿並正式送出；版本頁目前顯示「正在等待審查」。
- 版本採審核通過後手動發佈。

## 1.2.0／Build 3 送審結果

驗證日期：2026-07-28（Asia/Taipei）

- iOS／iPadOS／Apple Watch 與 macOS 的 1.2.0／Build 3 均已成功上傳並完成 Apple 處理。
- iOS 與 macOS 1.2.0 均已選入 Build 3。
- 繁體中文與英文（美國）的「此版本的新增功能」均已保存。
- Build 3 的出口合規答案均為未使用需申報的加密演算法。
- iOS／macOS 均維持審核通過後手動發佈，並保留現有評分。
- iOS／iPadOS／Apple Watch 1.2.0 已於 2026-07-28 12:10 提交，提交 ID：`ddd4c2dc-69b1-4910-9785-be4b6b9d37ac`，狀態為「等待審查」。
- macOS 1.2.0 已於 2026-07-28 12:09 提交，提交 ID：`1d7d71a6-de1e-4b2a-9cf9-61b5c24f2865`，狀態為「等待審查」。
