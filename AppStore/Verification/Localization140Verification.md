...
The repository-owned automated checks and four unsigned Release builds pass, and every built product now declares and packages the six release locales. Release acceptance is still blocked because:
...
| Verification tooling/fix commit | The commit containing this record, with subject `test: verify KnitNote 1.4 localization`; resolve its immutable SHA with `git log -1 --format=%H -- AppStore/Verification/Localization140Verification.md` |
...
The verification commit is intentionally not embedded as a literal SHA in its own contents because doing so would change that SHA. The final handoff must pair this record with the containing commit returned by Git.
...
| Final full Swift suite | PASS | 1,278 tests in 118 suites, zero failures |
...
| Release-audit localization behavior tests | PASS | 6 tests: missing `.lproj`, incomplete bundle declaration, incomplete source declaration, incomplete InfoPlist catalog, bad project regions, and valid six-locale fixture |
| Screenshot fixture/tooling tests | PASS | 12 tests |
...
```text
Test run with 1278 tests in 118 suites passed
METADATA CHECK: PASS
...
`InfoPlist.xcstrings` uses the String Catalog source-language/key fallback for the English `KnitNote Backup` source string; the other five locale values remain explicit and complete. The audit accepts that canonical source-language representation while rejecting any missing non-source locale.
...
After the minimal source-plist fix, the four targets were rebuilt from the same verification change set. The same four commands were run again from the final containing commit after it was created.
...
The new locale fixtures deliberately reuse neutral English sample user data. This prevents screenshots from implying that user-created project names, notes, journals, yarn details, or imported content are translated. It does not replace physical language-switching verification.
