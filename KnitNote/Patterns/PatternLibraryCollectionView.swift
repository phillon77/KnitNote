...
private struct PendingPatternMove: Identifiable {
    let patternID: UUID
    let currentFolderID: UUID?
    var id: UUID { patternID }
}

struct PatternLibraryCollectionView: View {
    ...
    @State private var pendingMove: PendingPatternMove?
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
                    ...
                    List {
                        ForEach(visibleRows) { row in
                            if let asset = asset(for: row.patternID) {
                                ...
                                .contextMenu {
                                    Button("patterns.folder.move", systemImage: "folder") {
                                        pendingMove = PendingPatternMove(
                                            patternID: row.patternID,
                                            currentFolderID: row.folderID
                                        )
                                    }
                                    .accessibilityLabel(Text("patterns.folder.move"))
            ...
            .sheet(isPresented: $addingYouTubeLink) {
                AddYouTubePatternView(
                    targetProjectID: nil,
                    targetFolderID: destinationFolderID
                ) { patternID, resolution in
    ...
    private var visibleRows: [PatternLibraryRowModel] {
        PatternLibraryIndex(rows: rows, locale: locale)
            .search(query, in: scope, sortedBy: sort)
    }

    private var destinationFolderID: UUID? {
        switch scope {
    ...
    private var scopeTitle: String {
        switch scope {
        case .all:
            LocaleAwareText.string("patterns.folder.all", locale: locale)
        case .uncategorized:
            LocaleAwareText.string("patterns.folder.uncategorized", locale: locale)
        case let .folder(folderID):
            store.patternFolders.first(where: { $0.id == folderID })?.displayName
                ?? LocaleAwareText.string("nav.patterns", locale: locale)
        }
    }
    ...
    private func chooseDuplicate(for selection: PendingPatternSelection) -> some View {
        NavigationStack {
