...
struct RenameProjectView: View {
    ...
    var body: some View { NavigationStack { Form { TextField("project.name", text: $name) }.navigationTitle("project.rename").toolbar {
    ...
    }}.frame(minWidth: 320, minHeight: 180).onAppear { name = store.project(id: projectID)?.name ?? "" } }
}
