...
struct StitchDetailView: View {
    ...
    private func text(_ key: String) -> String { CalculatorLocalization.string(key, locale: locale) }
    ...
    private func count(_ key: String, _ value: Int) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(StitchDictionaryText.format("stitchDictionary.count.value", locale: locale, Int64(value)))
                .preference(key: StitchDictionaryContentPreferenceKey.self, value: ["stitchDictionary.count." + key: StitchDictionaryText.format("stitchDictionary.count.value", locale: locale, Int64(value))])
        }
    }
