...
struct StitchDictionaryView: View {
    ...
    private func text(_ key: String) -> String { CalculatorLocalization.string("stitchDictionary." + key, locale: locale) }
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                if let errorKey = presentation.errorKey {
                    ContentUnavailableView {
                        Label(CalculatorLocalization.string(errorKey, locale: locale), systemImage: "exclamationmark.triangle")
                    } actions: {
                        Button(text("back")) { dismiss() }
                    }
                    .accessibilityIdentifier("stitchDictionary.error")
                    .preference(key: StitchDictionaryContentPreferenceKey.self, value: ["stitchDictionary.error": CalculatorLocalization.string(errorKey, locale: locale)])
                                ...
                                NavigationLink {
                                    ...
                                    CalculatorCard {
                                        VStack(alignment: .leading, spacing: 8) {
                                            Text(CalculatorLocalization.string(entry.titleKey, locale: locale)).font(.headline)
                                            ...
                                            Text(CalculatorLocalization.string(entry.summaryKey, locale: locale))
                                                .preference(key: StitchDictionaryContentPreferenceKey.self, value: ["stitchDictionary.entry." + entry.id: CalculatorLocalization.string(entry.summaryKey, locale: locale)])
                                            if let repetitions = result.repetitions {
                                                Text(StitchDictionaryText.format("stitchDictionary.repeat.format", locale: locale, Int64(repetitions)))
                                            }
                                            HStack {
                                                ForEach(entry.symbols) { symbol in
                                                    if let diagram = diagrams.first(where: { $0.id == symbol.diagramID }) {
                                                        StitchDiagramView(diagram: diagram, accessibilityText: CalculatorLocalization.string(symbol.traditionKey, locale: locale)).frame(width: 40, height: 40)
                                                    }
                                                }
                                            }
        ...
        }.background(CalculatorWatercolorBackground()).navigationTitle(text("title"))
    }
}
