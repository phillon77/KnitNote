...
@Suite struct BackupSettingsViewContractTests {
    ...
    @Test func createdLibraryImportShowsOneLocalOnlyReminderAndDismissalPersistsIt() throws {
        ...
        let composition = try source("KnitNote/App/AppSessionComposition.swift")
        let boundary = try source("KnitNote/App/AppSessionRootView.swift")
        #expect(app.contains("AppSessionComposition.makeLaunch("))
        #expect(composition.contains("PatternBackupReminderPresenter(history: backupHistory)"))
        #expect(boundary.contains(".environmentObject(presentation.patternBackupReminderPresenter)"))
