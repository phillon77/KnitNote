...
""", at: sectionEnd.lowerBound)
    ...
    @Test func archiveAuditRejectsPreviousVersionAcrossShippingProducts() throws {
        ...
        #expect(result.status != 0)
        #expect(result.output.contains("iOS product version is 1.5.1, expected 1.7.1"))
    }

    @Test func archiveAuditRejectsPreviousBuildAcrossShippingProducts() throws {
        let fixture = try makeArchiveFixture(version: "1.7.1", build: "12")
        defer { try? FileManager.default.removeItem(at: fixture.temporaryRoot) }
        ...
        #expect(result.status != 0)
        #expect(result.output.contains("iOS product build is 12, expected 16"))
    }
    ...
    @Test func creatorFixturePublishesPrivateCandidateWithoutRawPackagingLogsAndCleansRaceStaging() throws {
        ...
        #expect(!successfulRemaining.contains(where: { $0.hasPrefix(".KnitNote-1.7.1.staging.") || $0.hasPrefix(".KnitNote-1.7.1.worktree.") }))
        ...
        #expect(!remaining.contains(where: { $0.hasPrefix(".KnitNote-1.7.1.staging.") }))
        ...
        #expect(!cleanupRemaining.contains(where: { $0.hasPrefix(".KnitNote-1.7.1.staging.") || $0.hasPrefix(".KnitNote-1.7.1.worktree.") }))
        ...
        #expect(!mktempRemaining.contains(where: { $0.hasPrefix(".KnitNote-1.7.1.staging.") || $0.hasPrefix(".KnitNote-1.7.1.worktree.") }))
    }
