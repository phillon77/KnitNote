...
@Suite struct ReleaseConfigurationContractTests {
    @Test func projectUsesProductionIdentifiersVersionAndTeam() throws {
        ...
        #expect(
            yaml.components(separatedBy: "MARKETING_VERSION: 1.6.0").count == 4
        )
        #expect(
            yaml.components(separatedBy: "CURRENT_PROJECT_VERSION: 12").count == 4
        )
    ...
    @Test func releaseAuditUsesVersion160Build12AndHistoricalVerificationStaysLabeled() throws {
        ...
        #expect(audit.contains(#"EXPECTED_VERSION="1.6.0""#))
        #expect(audit.contains(#"EXPECTED_BUILD="12""#))
    ...
    @Test func staticReleaseAuditPinsBuildTwelveAndChecksEveryStringCatalog() throws {
        let script = try sourceText("AppStore/Verification/release_audit.sh")

        #expect(script.contains("EXPECTED_BUILD=\"12\""))
