...
@Suite struct StoreScreenshotFixturesTests {
    ...
    @Test func composerCreatesAContactSheetForEveryManifestLocale() throws {
        ...
        let provenanceSetup = try screenshotProcess(
            ...
            arguments: [
                "-c",
                "import sys; sys.path.insert(0, sys.argv[1]); from provenance import *; m=Path(sys.argv[2]); r=Path(sys.argv[3]); c='" + String(repeating: "a", count: 40) + "'; h='" + String(repeating: "b", count: 64) + "'; p={n:{'bundleIdentifier':PRODUCT_IDS[n],'version':'1.5.1','build':'11','sourceRevision':c,'executable':PRODUCT_EXECUTABLES[n],'executableSHA256':h} for n in PRODUCT_IDS}; atomic_write(r/'candidate-provenance.json', create_raw(m,r,c,'1.5.1','11',p))",
        ...
        let staleCandidateVerification = try screenshotProcess(
            ...
            arguments: [
                "-c",
                "import sys; sys.path.insert(0,sys.argv[1]); from provenance import verify_generated; from pathlib import Path; verify_generated(Path(sys.argv[2]),Path(sys.argv[3]),Path(sys.argv[4]),Path(sys.argv[5]),expected_commit='" + String(repeating: "c", count: 40) + "',expected_version='1.5.0',expected_build='10')",
                screenshotRepositoryRoot.appending(path: "AppStore/Screenshots").path,
                manifest.path,
                root.appending(path: "Raw").path,
    ...
    @Test func allLocaleCaptureIsAtomicAndRecordsTheCandidateCommit() throws {
        ...
        var result = try screenshotProcess(
            executable: "/bin/bash",
            arguments: ["AppStore/Screenshots/capture.sh", "--all-locales", commit, "1.5.1", "11"],
            environment: environment
        )
        #expect(result.status != 0)
        #expect(result.output.contains("production capture rejects KNITNOTE_SCREENSHOT_CAPTURE_RUNNER"))

        result = try screenshotProcess(executable: "/bin/bash", arguments: ["AppStore/Screenshots/capture.sh", "--test-only", "--all-locales", commit, "1.5.1", "11"], environment: environment)
        #expect(result.status == 0, Comment(rawValue: result.output))
    ...
    @Test func screenshotBuildInstructionsEmbedTheCandidateCommitInEveryProduct() throws {
        ...
        #expect(readme.contains("CANDIDATE_VERSION='1.5.1'"))
        #expect(readme.contains("CANDIDATE_BUILD='11'"))
        #expect(readme.contains("exact `1.5.1 (11)`"))
    }
    ...
    @Test func screenshotPythonRuntimeSkipsAnIncompatibleCandidate() throws {
        ...
        let wrongPillow = root.appending(path: "wrong-pillow-python")
        let compatible = root.appending(path: "compatible-python")
        try "#!/bin/sh\nexit 1\n".write(to: incompatible, atomically: true, encoding: .utf8)
        try "#!/bin/sh\nif [ \"$1\" = -c ]; then case \"$2\" in *'__version__'*) printf '11.2.0\\n' ;; esac; exit 0; fi\nprintf 'wrong:%s\\n' \"$*\"\n"
            .write(to: wrongPillow, atomically: true, encoding: .utf8)
        try "#!/bin/sh\nif [ \"$1\" = -c ]; then case \"$2\" in *'__version__'*) printf '11.3.0\\n' ;; esac; exit 0; fi\nprintf 'compatible:%s\\n' \"$*\"\n"
            .write(to: compatible, atomically: true, encoding: .utf8)
        for path in [incompatible, wrongPillow, compatible] {
            try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: path.path)
        }
        let launcher = screenshotRepositoryRoot.appending(path: "AppStore/Screenshots/python_runtime.sh")
        let result = try screenshotProcess(
            executable: "/bin/bash",
            arguments: ["-c", "source \"$1\"; selected=$(select_screenshot_python \"$2\" \"$3\" \"$4\"); \"$selected\" payload", "test", launcher.path, incompatible.path, wrongPillow.path, compatible.path]
        )
        #expect(result.status == 0)
