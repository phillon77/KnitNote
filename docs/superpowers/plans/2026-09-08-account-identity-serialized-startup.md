...
- [x] **Step 1: Add focused RED tests using controlled closures.** Use these behavioral cases, not source-string-only checks:
...
Actual reopen RED requires narrow `CloudSyncEngineTransport.swift` receipt accounting and existing transport tests as described below; no Core/incoming format changes.
...
var statusDidChangeHandler: (() -> Void)? { get set }
```
...
The stop/join and status callback seams remain on the same coordinator: synchronous invalidate, retain transport teardown, join controller pump before lifecycle freeze. Status callbacks must tolerate synchronous subscriber stop/account reentry and reflect actual local access while initial fetch remains suspended. Reused Session callbacks capture a new runtime-generation token, so toggling isCurrent on the same Session cannot revive abandoned engine closures.

Review correction: requestIdentity revalidates stopped and its captured request generation after every synchronous lifecycle/coordinator callout, not only within the state setter; owner-visibleSession hide subscribers may stop before outer checking assignment. Add that exact regression. The sync coordinator's independently spawned startup Task and eventLoopTask must also be retained and joined through existing KnitNoteCloudSyncCoordinator before Session replacement/freeze/cleanup. Transition waiter completion is not actual startup/event completion. A callback may initiate stop but must not await its own event loop; the external session owner performs the join. Test suspended initial fetch after driver cancellation completes, and suspended actual committer attachment resolution before store admission, so neither driver cancel nor store drain alone can satisfy shutdown. This authorizes narrow KnitNoteCloudSyncCoordinator.swift and existing sync tests, no new cleanup authority or Core policy.

Successful same-account empty fetch RED exposes deduplicated acknowledged incoming redelivery: FileCloudIncomingBatchStore.record can return fullyObservedBatchIDs with deliveredEnvelope=nil, whereas transport currently adds receipt membership only for new envelopes. Repair receipt accounting for exact current-request fully observed batch identities; only existing exact account/zone durable ACK verification can count them committed. Unacknowledged replay still waits for actual domain commit/ACK; partial/invalid/stale evidence cannot issue receipt. Preserve nonempty committedFetchReceipt checks, incoming dedup/format/retirement and source-observation authority. Do not create dummy batches or equate observation with commitment. Test repeated empty restart, populated ACKed redelivery, unACKed/partial/invalid-proof gates and stale epoch, with final settled state assertions beyond early localReady.

- [x] **Step 1: Add actual controlled RED sequences.** Construct actual AppSessionOwner/factory/lifecycle/coordinator with existing temporary-root fixture and in-memory vault keychain. Query closure controlled by continuations, engine uses existing driver. Assert:
...
- [x] **Step 4: Focused validation and commit.** Run all new controller and affected actual lifecycle/coordinator/owner tests under the actual-source harness. Mutation RED: remove query-generation acceptance or same-account reuse and observe a real stale publication/extra seal failure, restore code. Record source inventory, commands/log hashes, unchanged Core/data policy, and exact commits for independent task review. Keep full Core/platform chain for Task3.
...
- [x] **Step 1: Independent whole-plan review.** Use exact baseline through final task HEAD and reports, then adjudicate current task findings; carry canonical-probe Minor1/live-helper follow-ups rather than silently closing them.
