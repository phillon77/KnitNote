...
**Execution mode:** Immediate scoped work only; automatic delegation remains PAUSED. Default handoff is inline execution in the existing worktree. No new agent, automation, App activation, signing, push or submission is authorized by this document.
...
- [ ] Run `owned_core_test ProjectArchiveSyncMapperTests`: initial RED is missing `projectUnvalidated`; after the extraction expect all selected tests green.
- [ ] Add separate runtime ordering regressions at the shared walk boundary: missing source, unstaged source and changed physical bytes must each precede that slot's invalid destination. Retain the ordinary loop traversal order; do not sort it as part of this extraction.
- [ ] Extend the same parity test to `BackupFixture.writePatternLibraryArchive(to:includeLinkedYarn:)`, missing required media, label photos, optional markup and empty records. Reuse actual fixtures, never manufacture staged authority in production.
...
This is an execution plan, not proof of implementation or release readiness. The first execution step is Task 1A's failing mapper test; retain its API RED, then establish runtime ordering regressions before refactoring. Run the four checkpoints inline under scoped user approval. Do not restart automatic delegation.
