...
這個stop／drain對新activation／send／reply的禁止與等待，只涵蓋coordinator發起或已接受的工作。清除callback不會停止或等待`PhoneWatchSession`原生adapter自有的Task：包含`sessionDidDeactivate`後的重新activation，以及收件FIFO在callback已清空後仍可能回覆空失敗結果。未來App owner整合必須另行定義本機原生adapter的admission／stop／drain擁有權；這和遠端Watch清空、account session證據與wire協定關卡是分開的界線。本段不宣稱原生transport完全靜止。
...
2. Watch同步入列後立即stop，coordinator已接受的舊command不執行、coordinator不再發起回覆或傳送；另存的四種callbacks、訂閱晚到回呼與timer在stop後無coordinator傳送或落盤。stop前既有正常snapshot／指令語意保留。`PhoneWatchSession`原生adapter的callback／FIFO／重新activation生命期不在此測試收據內，待未來App owner以獨立的本機adapter gate驗收。
