...
日期：2026-09-07。狀態：設計草案，等待書面設計確認；尚未修改正式程式。
...
設計自查：所有 Task 有同一 admission/drain 所有權；FIFO 不更改 wire；停止與同步重入明確；A 不能移除 B delegate；外部未回覆傳輸不阻塞本機 drain；平台測試與實機證據不混用。待使用者確認本文件後，再撰寫分步實作計畫並執行。
