...
struct PreparedBody {
    ...
    let sourceControlSHA256: Data? // exact preparing source control; nil omitted only for archive route
    let formerSourceSHA256: Data? // full canonical prior source state; required32 for missingArchive, omitted for archive
    let pendingSnapshotSHA256: Data // exact preparing source-and-pending baseline, required32 bytes
    let immutableOutputSHA256: Data // required32: complete portable immutable-role proof map
...
PreparedBody retains the exact preparing sourceControlSHA256 and pendingSnapshotSHA256 in every subsequent phase and normalized prepared digest. The preparation hash alone cannot reconstruct those witnesses after restart. A missing-archive prepared source requires the nonnil32-byte original control digest; archive-route nil is omitted, never explicit null. The source-and-pending baseline is required32 bytes. Before-spend recovery must compare actual original control as well as source/pending evidence, rejecting a same-baseline different generation. This is a strict extension of the not-yet-activated owned v3 wire; old v1/v2 is unchanged and no missing-witness compatibility bypass is permitted. Actual shared codec lifetime accounting includes both fields.

PreparedBody.formerSourceSHA256 additionally binds the complete canonical former SyncAccountSourceState, including generation, authority identifier, origin and all account/root/baseline fields. It is required32 for missingArchive and omitted (not null) for archive. Derive it only from actual old control bytes still matching durable preparing.sourceControlSHA256; recheck that control before publishing prepared. No additional preparing witness is needed. Carry the digest through normalized prepared and every derived phase, actual budget encodings, installation/recovery/commit and capture/authenticated decode validation. A spent predecessor hash is not proof that the spent payload's source is unchanged: the full source digest must also match. Terminal-origin handoff establishes a new real source for the next attempt rather than reusing old identity. Strict owned-v3 decoding rejects absent, null, malformed or source-kind-inconsistent witnesses; old v1/v2 remains unchanged.
...
// The existing Original subtree has a freshly verified physical identity,
// pinned within each owner scope; its prior inode is not persisted here.
...
PreparedBody.immutableOutputSHA256 binds the complete canonical portable proof map of Original, Attachments, ValidationOriginal and ValidationMerged, including exact role directories, all descendants and absence. Paths are exact UTF-8 relative names with type, byte count and content hash; no arbitrary role is admitted. Exclude movable Staged/Displaced/Failed, active/history files and mutable UUID-root metadata. Compute from actual synchronized outputs before prepared publication, retain in every derived phase and normalized digest, and include the required32 field in actual lifetime accounting. Recompute before reopened moves and validate against captured/frozen/history entries on their respective paths. Missing, null, wrong-length or pre-extension owned-v3 bodies reject; legacy v1/v2 remains unchanged. This binds unreferenced attachments and validation outputs as well as selected sources.

Before terminal freeze, the immutable-role digest does not preserve prior device/inode values. Same-byte replacement of an unpersisted Original/Attachments/Validation inode across restart is therefore not detectable as historical inode replacement. Every reopened owner must first validate the complete physical no-follow tree and portable proofs, then pin actual immutable entries throughout its scope, rejecting callback-time replacement. Persisted originalLiveRoot/stagedRoot identities, historical UUID treeEntries and terminal frozen entries retain their stronger cross-restart identity checks. Portable hashes never grant deletion or restore-target ownership. This explicit limit does not permit extra paths, changed bytes, aliases or refreezing modified terminal evidence.
