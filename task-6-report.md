...
## Smart reminders Task 6 — Watch schema 4 and optimistic projections

### RED

- `swift test --disable-sandbox --filter 'WatchSyncModelsTests|WatchOptimisticStateTests|WatchReliableSnapshotFingerprintTests'` initially failed at compile time for the absent v4 reminder/occurrence snapshots, action payload, and new reminder operations.

### GREEN

- Replaced new Watch projection output with schema 4 project-level `knittingReminders`; it carries stable reminder/counter/occurrence identity, semantic kind, verbatim text, rule, state, progress, pending occurrence display/phase state, and mutation revision. Snapshot schema 3 and command schema 2 still decode explicitly for recovery; future/malformed versions reject.
- Command schema 3 requires exactly one `WatchReminderActionPayload` for complete/defer/skip and rejects payloads on counter operations. Existing legacy reminder operation values remain decode-only compatibility cases while Task 7 owns durable application.
- The builder projects every project reminder, including secondary-counter migrations, sorted by Core creation ordering. The reliable fingerprint contains deterministic reminder and occurrence state.
- Optimistic state validates counter/reminder/occurrence/revision, applies complete/defer/skip only to the matched known occurrence, releases a deferred occurrence after a known increment, avoids generating IDs/revisions, and replays pending commands over authoritative replacements.

### Verification

- Focused GREEN: `swift test --disable-sandbox --filter 'WatchSyncModelsTests|WatchOptimisticStateTests|WatchReliableSnapshotFingerprintTests'` — 62 tests in 3 suites passed.
- Adjacent compatibility: `swift test --disable-sandbox --filter 'WatchCommandApplicationTests|WatchConnectivityEnvelopeTests|WatchSyncPersistenceTests|PhoneWatchSyncSourceContractTests'` — 68 tests ran; 3 existing Task 7 legacy durable-reminder tests (4 assertions) remain red because durable complete/defer/skip application is intentionally unimplemented in this task.
- watchOS compile: `xcodebuild -project KnitNote.xcodeproj -scheme KnitNoteWatch -destination 'generic/platform=watchOS Simulator' CODE_SIGNING_ALLOWED=NO build` passed.
- `git diff --check` passed.

### Scope and concern

- `JSONProjectStore`, `PreparedWatchCommand`, and the old Watch counter view changed only for exhaustive handling of the new operation cases; they do not apply reminder actions durably or add a Watch reminder UI. Task 7 and Task 8 retain those responsibilities.
